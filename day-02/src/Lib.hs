...
process :: String -> Int
process contents =
  let list = filter (/= []) (map (map read . words) (lines contents))
   in countSafeLines list

process' :: String -> Int
process' contents =
  let list = map (map read . words) (lines contents)
   in let countSafeLines' = length . filter isSafe'
       in countSafeLines' list
